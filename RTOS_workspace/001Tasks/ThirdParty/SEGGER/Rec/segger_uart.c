...
#define OFF_AHB2ENR         0x34        // AHB2 peripheral clock enable register
...
#define GPIO_UART_RX_BIT    7                // USART1 RX: Pin pb7
...
void HIF_UART_Init(uint32_t Baudrate, UART_ON_TX_FUNC_P cbOnTx, UART_ON_RX_FUNC_P cbOnRx) {
  ...
  uint32_t Div;
  //
  // Configure USART RX/TX pins for alternate function
  //
  RCC_APB2ENR |= (1 <<  4);       	// Enable USART1 clock
  RCC_AHB1ENR |= (1 <<  0);        	// Enable IO port A clock
  RCC_AHB1ENR |= (1 <<  1);        	// Enable IO port B clock

  uint32_t alternate_fn = 7;
  uint32_t reset_value = 0xF;
  v  = GPIOA_AFRH;
  uint32_t mask = (reset_value << ((GPIO_UART_TX_BIT - 8) << 2));
  v &= ~mask;
  v |=   (alternate_fn << ((GPIO_UART_TX_BIT - 8) << 2));
  GPIOA_AFRH = v;

  v  = GPIOB_AFRL;
  v &= ~(reset_value << (GPIO_UART_RX_BIT << 2));
  v |=   (alternate_fn << (GPIO_UART_RX_BIT << 2));
  GPIOB_AFRL = v;

  //
  // Configure USART RX/TX pins for alternate function usage
  //
  uint32_t alternate_fn_mode = 2;
  reset_value = 3;
  v  = GPIOA_MODER;
  v &= ~(reset_value << (GPIO_UART_TX_BIT << 1));
  v |=  (alternate_fn_mode << (GPIO_UART_TX_BIT << 1));         // PA9: alternate function
  GPIOA_MODER = v;

  v  = GPIOB_MODER;
  v &= ~(reset_value << (GPIO_UART_RX_BIT << 1));
  v |=  (alternate_fn_mode << (GPIO_UART_RX_BIT << 1));         // PB7: alternate function
	GPIOB_MODER = v;
  //
  // Initialize USART
  //
